...
def test_with_rgbd(net, cfg, rgb_path, has_label):
...
def main():
    ...
    model_ckp_name = "../logs/dataset_newnew/rgbd/model.chk-100"
    ...
    test_path = "/data/tmp/hl_data/validation/rgbd/test/all/"
    rgbd_images = list(glob(os.path.join(test_path,  '*.npy')))
    # np.random.shuffle(rgb_images)

    nr = rgbd_images.index(test_path+str(sys.argv[1]))
    # print(nr)
    image = rgbd_images[nr]
    test_with_rgbd(net, cfg, image, False)
    '''
    start = time.time()
    iterations = 100
