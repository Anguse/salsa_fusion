...
def test_with_rgb(net, cfg, rgb_path, has_label):
    ...
    rgb_img = rgb.astype('float32') / 255
    pimg.imsave('../preds/rgb_orig.png', rgb_img)

    with tf.Session(graph=net.graph) as sess:
        net.initialize_vars(sess)

        pred_img = net.predict_single_image(input_img=rgb_img, session=sess)
        '''
        ...
        for i in range(0,cfg.NUM_CLASS):
            s = cfg.CLASSES[i] + " PREC: {: 3.3f} " + cfg.CLASSES[i] + " REC: {: 3.3f} " + cfg.CLASSES[i] + " IOU: {: 3.3f}"
        ...
        if has_label:
            label = rgb[:, :, 3]
            pimg.imsave('../preds/rgb_label.png', label)
            ax2.axis('off')
            ax2.imshow(label)
        ...
        pimg.imsave('../preds/rgb_pred.png', pred_img)
        # showPredImg(pred_img)
        #np.save('../preds/depth_pred.npy', pred_img)


def test_with_depth(net, cfg, depth_path, has_label):
...
def test_with_batches(net, cfg, test_path, iterations):
    with tf.Session(graph=net.graph) as sess:
        net.initialize_vars(sess)
        ...
        for i in range(iterations):
            ...
            test_ious = np.array(test_ious[test_ious != 0])
...
def main():
    ...
    model_ckp_name = "../logs/dataset_newnew/rgb/model.chk-100"
    ...
    #test_path = "/data/tmp/hl_data/dataset/rgb/test/"
    test_path = "/data/tmp/hl_data/validation/rgb/test/all/"
    rgb_images = list(glob(os.path.join(test_path,  '*.npy')))
    # np.random.shuffle(rgb_images)

    nr = rgb_images.index(test_path+str(sys.argv[1]))
    # print(nr)
    image = rgb_images[nr]
    test_with_rgb(net, cfg, image, False)
    '''
    start = time.time()
    iterations = 100
    score, n_test_samples = test_with_batches(
    ...
    #test_with_rgb(net, cfg, image, has_label=False)
    '''
